...
public class FusionTableSample {
     ...
     */
    private static final String APPLICATION_NAME = "ECEN689";
    ...
    public static void main(String[] args) {
        try {
            ...
            String tableId = getTableId("fortune150"); //make sure to use exact spelling for this method
    ...
    private static Credential authorize() throws Exception {
        ...
        GoogleClientSecrets clientSecrets = GoogleClientSecrets.load(
                JSON_FACTORY, new InputStreamReader(
                        FusionTableSample.class.getResourceAsStream("/client_secret.json")));
    ...
    private static void insertData(String tableId) throws IOException {
        ...
        Sql sql2 = fusiontables.query().sql("INSERT INTO " + tableId + " (Text,Number,Location,Date) "
                + "VALUES (" + "'Google Inc', " + "1, " + "'1600 Amphitheatre Parkway Mountain View, "
                + "CA 94043, USA','" + new DateTime(new Date()) + "')");
        Sql sql = fusiontables.query().sql("INSERT INTO " + tableId + " (NAME,R2014,R2013,CEO,address) "
                + "VALUES (" + "'Wonture Inc', " + "1414, "+"1313, " +"'Akash Sahoo',"+ "'507 Nagle St, "
                + "TX 77840, USA'"+")");
    ...
    private static void deleteRows(String tableId) throws IOException {
    	int ROWID = 146;
        Sql sql = fusiontables.query().sql("DELETE FROM " + tableId+" { WHERE ROWID = '"+ROWID+"'}");

        try {
            sql.execute();
           //  sql = fusiontables.query().sql("DELETE FROM " + tableId+" WHERE ROWID = 152");
            // sql.execute();
        } catch (IllegalArgumentException e) {
            // For google-api-services-fusiontables-v1-rev1-1.7.2-beta this exception will always
            // been thrown.
